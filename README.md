...
- **移动端**：封面区域左右滑动翻书；地区/县级下拉在面板标题下方选柜，
  大陆地图可左右滑动看全图。
...
covers/                封面图（c001…c167，条目用「封面：covers/cXXX.jpg」引用）
...
- **发「收录上新」横幅**：编辑 `template.html` 里的 `UPDATES` 数组（往数组头部加批次）。
  ...
  完全一致（含括号后缀），地域名必须与书柜 region 全名一致，`validate.py` 都会校验。
  横幅常驻显示最近一次批次，直到下一次收录新刊时以新批次覆盖；
  批次日期必须写「对外发布当天」——收录与发布不同日时，发布前同步更新日期。
...
- **信源官方优先**：官网/出版方页/官方商店排前，媒体报道等二手仅作补充；
  显示名用网站/机构/报道名，不用裸域名。
- **gov.tw 等被拦站点：本机打不开 ≠ 信源失效**，先按 docs/research.md 的
  CI 机房实测法判定，可达的通道一律保留（结论固化在 tools/fetch_issues.py 文档串）。
- **书柜内排序（全部地图通用）**：新增刊物落位时——持续发行在前，已停刊在后，
  单册（如有）最后；创刊时间不确定的排在其所属组别（持续发行/已停刊）的末位。
  既有相邻顺序不强制追溯重排。
- **新增刊物提交前必须逐项验证现有机制**（2026-09 食通信批次起执行）：
  ① `build.py --check` 与 `validate.py` 全绿；② 书柜内排序（全图通用）——持续发行在前、已停刊次之、
  单册（如有）最后，创刊时间不确定者在所属组内靠后；③ 硬编码计数全部同步——档案头部统计、
  「全国书柜」圆片 rc-n、面板书柜速览的「N 种刊物」「流动刊物 N 种」；
  ④ 收录上新批次日期为当天、地域名与书柜全名一致；⑤ 涉及地图取景、pin、标签的
  改动，桌面与移动（390×844）各截图核验一遍（方法见 docs/mobile.md）。
- **批量编辑先核原文再替换**：批量改动前先 `grep` 确认该处的实际文本（简繁转换、
  标点全半角都可能让「以为的原文」不存在），替换用「断言唯一命中」的脚本方式防误伤。

## 移动端适配约定

- 悬浮角标（补/赞）竖排右下，触控目标 ≥44px，避让「全国书柜」圆片与翻页器。
- 地区/县级下拉统一在面板标题正下方；台湾、日本用专用下拉（wireJump 接线，
  含 hash 深链同步），其余面板移动端自动生成。
- 大陆地图移动端为 700px 横滑画布，默认滚动停东部书柜密集团（大理贴左缘约 8%），
  右滑可看拉萨/新疆；桌面取全图。
- 弹窗内媒体用 min(宽px, 74vw) 纵向堆叠；页面底部留白约 45px（桌面 60px）。
- 改动前先读 docs/mobile.md（原则、时序坑、验证流程）；
  线上核实事实的工具与方法见 docs/research.md。
