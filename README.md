...
- **书柜内排序（全部地图通用）**：持续发行在前，已停刊在后；创刊年份不详且
  已停刊的排最后。新增刊物后必须按此重排（2026-09 起执行并全图校准一轮）。
- **新增刊物提交前必须逐项验证现有机制**（2026-09 食通信批次起执行）：
  ① `build.py --check` 与 `validate.py` 全绿；② 书柜内排序（全图通用）——持续发行在前、
  已停刊在后、停刊且创刊年不详最后；③ 硬编码计数全部同步——档案头部统计、
