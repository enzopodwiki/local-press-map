...
def main():
    ...
    for r in data:
        pat = re.search(r'<g class="pin[^"]*" data-r="' + re.escape(r['id']) +
                        r'"[^>]*>.*?<text class="pcount[^"]*" y="[^"]*">(\d+)</text>', html, re.S)
        if r.get('roam'):
            if pat:
    ...
    # 6. 「收录上新」：书柜 id、地域名、书名都要能对上（点击会深链跳到具体那本书）
    ...
    for m in re.finditer(r"\['([a-z0-9-]+)',\s*'([^']*)',\s*'(.*?)'\]", updates):
        ...
        g = by_id.get(rid)
        if not g:
            err(f'「收录上新」引用了不存在的书柜 id：{rid}')
            continue
        if label != g['region']:
            note(f'「收录上新」地域名与数据不一致：{rid} 写的是「{label}」，书柜实际叫「{g["region"]}」')
        if not any(p['t'] == title for p in g['pubs']):
            err(f'「收录上新」条目在书柜 {rid}（{g["region"]}）中找不到：{title}')
